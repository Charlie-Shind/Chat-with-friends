<template>
	<view class="">
				...
				<view class="u-flex u-row-between u-m-t-20 u-font-24 detail-tag">
					<view class="u-flex">
						...
						<view class="">
							...
							<text class="u-m-l-10 u-m-r-5" v-text="archivesInfo.likes"></text>
						</view>
						<view class="u-m-l-30">
							<u-icon name="chat-fill" color="#aaa" size="20"></u-icon>
							<text class="u-m-l-10 u-m-r-5" v-text="archivesInfo.comments"></text>
						</view>
						<view class="u-m-l-30">
							<u-icon name="eye-fill" color="#aaa" size="20"></u-icon>
							<text class="u-m-l-10 u-m-r-5" v-text="archivesInfo.views"></text>
						</view>
					</view>
					<view class="">
						<!-- #ifdef MP-WEIXIN -->
		...
		<u-modal v-model="copyshow" confirmText="复制下载链接" @confirm="confirmCopy">
			<view class="u-p-30">
				<view class="u-p-20">当前平台不支持下载该文件，请复制下载链接后前往浏览器进行下载</view>
			</view>
		</u-modal>
		<!-- 回到顶部 -->
		...
		methods: {
			...
			download(item) {
				...
				switch (item.name) {
					case 'local':
						// #ifndef H5
						if (item.url.match(/\.(doc|xls|ppt|pdf|docx|xlsx|pptx)$/i)) {
							uni.downloadFile({
								url: item.url,
								success: res => {
			...
			confirmCopy() {
				this.$util.uniCopy({
					content: this.downurl,
					success: () => {
						this.$u.toast('下载链接复制成功！');
					}
				});
			},
			//跳转标签页
